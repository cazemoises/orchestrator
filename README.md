...
| `ORCH_LOCAL_VERIFY_COMMAND` | não | (vazio = pula a verificação, não recomendado) | Comando rodado via `sh -c` em `ORCH_REPO_DIR` antes de todo commit+push. Ex: `go build ./... && go vet ./...` quando `ORCH_REPO_DIR` for `pdf-reader/backend`; ajuste para `npm run build` (frontend) ou o equivalente do extractor conforme o backlog cobrir outras partes do repo. |
